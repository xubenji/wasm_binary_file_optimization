...
###
 ...
 # @LastEditTime: 2023-03-08 14:29:24
...
while read line; do
    ...
    # 写入输出文件
    echo $merged_line >> t4_merged_output.txt
    # 写入新文件
    #echo $merged_line2 >> 2_merged_output2.txt
...
#     # 获取本行和下一行的所有内容
#     col1_and_rest=$(echo $line | cut -d ' ' -f 1-)
#     next_col1_and_rest=$(echo $next_line | cut -d ' ' -f 1-)
#     # 将第一列字符串合并到一行，并将空格替换为其他字符
#     merged_line="$col1-$next_col1 $col1_and_rest $next_col1_and_rest"
#     # 写入新文件
